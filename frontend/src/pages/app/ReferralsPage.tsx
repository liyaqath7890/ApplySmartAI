      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <div className="bg-gradient-to-br from-primary-50 to-blue-50 rounded-xl border border-primary-200 p-6">
            <h3 className="text-lg font-semibold text-app-primary mb-4">Referral Stats</h3>
        ...
        <div className="lg:col-span-2 space-y-4">
          {referrals.length === 0 ? (
            ...
            referrals.map((referral) => (
              <div key={referral.id} className="bg-app-card rounded-xl border border-app-border p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-app-primary">{referral.role}</h3>
                    <p className="text-sm text-app-secondary">{referral.company}</p>
                    {referral.location && <p className="text-xs text-app-secondary mt-1">{referral.location}</p>}
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <span className="px-3 py-1 bg-app-subtle text-app-primary rounded-full text-sm font-medium">{referral.bonus}</span>
                    <Badge variant={statusVariant(referral.status)}>{referral.status}</Badge>
                  </div>
                </div>
                <div className="flex items-center gap-4 mb-4">
                  <div className="flex items-center gap-2 text-sm text-app-secondary"><Users className="h-4 w-4" /><span>Referrer: {referral.referrer}</span></div>
                  <div className="text-sm text-app-secondary">{referral.posted}</div>
                </div>
