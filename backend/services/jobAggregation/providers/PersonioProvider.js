...
/**
 * Personio Careers Provider
 * URL: https://<companyId>.personio.de/xml
 */
export class PersonioProvider extends BaseATSProvider {
  constructor(config = {}) {
    super(config);
  ...
  async fetchCompanyJobs(company, options = {}) {
    ...
    try {
      const url = `https://${companyId}.personio.de/xml`;
      ...
      this.log(`Error fetching Personio jobs for ${companyId}: ${error.message}`, 'error');
      return [];
    }
  }
